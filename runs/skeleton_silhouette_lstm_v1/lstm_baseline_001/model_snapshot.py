"""Skeleton + silhouette early-fusion LSTM baseline.

Comparison baseline for `skeleton_silhouette_fusion_v6`. Instead of V6's
learned gated fusion, part-based pooling, and multi-branch temporal CNN +
attention pooling, this design uses the simplest reasonable pipeline:

  early channel concatenation -> plain CNN frame encoder -> bidirectional
  LSTM -> mean pooling -> embedding head.
...
class LSTMBaseline(nn.Module):
    ...
    def forward(self, silhouette, topology, reconstruct: bool = False, mask_ratio: float = 0.0):
        ...
        temporal, _ = self.temporal_rnn(frame_features)
...
def build_model(config, num_classes: int) -> LSTMBaseline:
    return LSTMBaseline(
